...
from parking_monitor.db.repository import ParkingRepository, ValidationError, NotFoundError, RepositoryError
...
class AdminGUI:
    ...
    def __init__(self, root: tk.Tk, db_repo: ParkingRepository):
        ...
        self.current_camera_info: Optional[Camera] = None
    ...
    def _load_cameras(self, page: int = 1, page_size: int = 100):
        """Загружает список камер из БД с пагинацией"""
        self.camera_listbox.delete(0, tk.END)

        try:
            offset = (page - 1) * page_size
            cameras = self.db.get_all_cameras(limit=page_size, offset=offset)
            ...
            self.status_var.set(f"Загружено {len(cameras)} камер")

        except Exception as e:
            self.status_var.set(f"Ошибка загрузки камер: {e}")
    ...
    def _load_camera_data(self, camera_id: int):
        ...
        try:
            ...
            if not camera:
                self.status_var.set(f"Камера {camera_id} не найдена")
                return
            ...
            self.cam_info_text.insert(1.0,
                                      ...
                                      f"Активна: {'Да' if camera.is_active else 'Нет'}"
                                      )
            self.cam_info_text.config(state=tk.DISABLED)
            ...
            self.status_var.set(f"Ошибка загрузки камеры: {e}")

    def _create_temp_scene(self, camera_id: int):
        """Создает временную 3D сцену с контейнерами из БД"""
        self.temp_scene = Scene3D(camera_id=camera_id)
        ...
        try:
            ...
            for container in containers:
                # Добавляем в сцену
                self.temp_scene.containers[container.id] = container

                # Если это базовый, запоминаем
                if container.is_base:
                    self.temp_scene.base_container_id = container.id

                    # Загружаем калибровку для базового контейнера
                    calibration = self.db.get_camera_calibration(container.id)
                    if calibration:
                        # Конвертируем из списков в numpy
            ...
            self.status_var.set(f"Ошибка создания сцены: {e}")
        ...
        try:
            ...
            if ret:
                ...
                if self.image_shape is None:
                    self.image_shape = (frame.shape[1], frame.shape[0])
                self._refresh_view()
    ...
    def _create_container_from_points(self):
        ...
        # Спрашиваем имя
        name = simpledialog.askstring("Имя места", "Введите название места (например A12):",
                                      parent=self.root)
        if not name:
            return
    ...
    def _save_all_to_db(self):
        ...
        if not self.temp_scene or not self.current_camera_id:
            messagebox.showwarning("Предупреждение", "Нет активной камеры для сохранения")
            return
        ...
        if self.temp_scene.base_container_id is None:
            if not messagebox.askyesno(
                    "Предупреждение",
                    "Отсутствует базовый контейнер (система координат).\n"
                    "Сохранить без базового контейнера? Это может нарушить 3D проекцию."
            ):
                return
        ...
        try:
            ...
            temp_containers = self.temp_scene.containers  # словарь

            # Собираем ID контейнеров
            temp_ids = set(temp_containers.keys())
            db_ids = set(db_containers.keys())

            # Список для отслеживания результатов
            saved_containers = []

            # 1. Подготавливаем операции: создание и обновление
            # Сначала обрабатываем все контейнеры, но не модифицируем temp_scene внутри цикла
            new_id_mapping = {}  # old_id -> new_id для созданных контейнеров

            for container_id, container in temp_containers.items():
                try:
                    is_base = (container_id == self.temp_scene.base_container_id)

                    if container_id not in db_containers:
                        # === СОЗДАНИЕ НОВОГО КОНТЕЙНЕРА ===
                        new_container = self.db.add_parking_container(
                        ...
                        saved_containers.append(("создан", new_container.id, container.name))
                        new_id_mapping[container_id] = new_container.id

                        # Сохраняем калибровку, если это базовый
                        if is_base and self.temp_scene.camera_calibration:
                            self._save_calibration(new_container.id, self.temp_scene.camera_calibration)

                    else:
                        # === ОБНОВЛЕНИЕ СУЩЕСТВУЮЩЕГО ===
                        db_container = db_containers[container_id]
                        update_data = {
                            'name': container.name,
                            'length': container.length,
                            'width': container.width,
                        ...
                        if container.image_points is not None:
                            update_data['image_points'] = container.image_points

                        if db_container.is_base != is_base:
                            update_data['is_base'] = is_base
                            if is_base and self.temp_scene.camera_calibration:
                                self._save_calibration(container_id, self.temp_scene.camera_calibration)
                            elif not is_base:
                                try:
                                    self.db.delete_camera_calibration(container_id)
                                except Exception:
                                    pass

                        self.db.update_parking_container(container_id, **update_data)
                        saved_containers.append(("обновлен", container_id, container.name))

                except Exception as e:
                    raise RepositoryError(f"Не удалось сохранить контейнер {container.name}: {e}")

            # 2. Удаляем контейнеры, которых нет во временной сцене
            containers_to_delete = db_ids - temp_ids
            for container_id in containers_to_delete:
                try:
                    self.db.delete_camera_calibration(container_id)
                    self.db.delete_parking_container(container_id)
                    saved_containers.append(("удален", container_id, db_containers[container_id].name))
                except Exception as e:
                    raise RepositoryError(f"Не удалось удалить контейнер {container_id}: {e}")

            # 3. Обновляем временную сцену: заменяем старые ID на новые
            if new_id_mapping:
                new_containers = {}
                for old_id, container in temp_containers.items():
                    new_id = new_id_mapping.get(old_id, old_id)
                    container.id = new_id
                    new_containers[new_id] = container
                self.temp_scene.containers = new_containers

                # Если базовый контейнер был создан заново, обновляем base_container_id
                old_base = self.temp_scene.base_container_id
                if old_base in new_id_mapping:
                    self.temp_scene.base_container_id = new_id_mapping[old_base]

            # 4. Обновляем калибровку для базового контейнера (если изменилась)
            if self.temp_scene.base_container_id is not None:
                base_container = self.temp_scene.containers.get(self.temp_scene.base_container_id)
                if base_container and self.temp_scene.camera_calibration:
                    self._save_calibration(self.temp_scene.base_container_id, self.temp_scene.camera_calibration)

            # 5. Перезагружаем сцену из БД для синхронизации
            ...
            # Формируем сообщение
            summary = [f"  • {action}: {name} (ID: {cid})" for action, cid, name in saved_containers]
            message = f"Изменения сохранены успешно!\n\n" + "\n".join(summary) if summary else "Изменений не обнаружено"
            self.status_var.set(f"Сохранено {len(saved_containers)} изменений")
            messagebox.showinfo("Успех", message)

        except ValidationError as e:
            messagebox.showerror("Ошибка валидации", f"Проверьте корректность данных:\n{str(e)}")
        except NotFoundError as e:
            messagebox.showerror("Не найдено", f"Объект не найден в базе данных:\n{str(e)}")
        except RepositoryError as e:
            messagebox.showerror("Ошибка базы данных", f"Не удалось сохранить изменения:\n{str(e)}")
        except Exception as e:
            messagebox.showerror("Ошибка", f"Непредвиденная ошибка:\n{str(e)}")
            import traceback
            traceback.print_exc()

    def _save_calibration(self, container_id: int, calibration: CameraCalibration):
        """
        Сохраняет или обновляет калибровку камеры для указанного контейнера.
        """
        try:
            homography = None
            if self.temp_scene and self.temp_scene._base_homography is not None:
                homography = self.temp_scene._base_homography

            # Пытаемся обновить существующую калибровку
            self.db.update_camera_calibration(
            ...
            # logger.info(f"Калибровка обновлена для контейнера {container_id}")
        except NotFoundError:
            # Калибровка не существует, создаем новую
            self.db.add_camera_calibration(
                container_id=container_id,
                camera_matrix=calibration.camera_matrix,
                rvec=calibration.rvec,
                tvec=calibration.tvec,
            ...
            # logger.info(f"Калибровка создана для контейнера {container_id}")
        except ValidationError as e:
            # logger.error(f"Ошибка валидации калибровки для контейнера {container_id}: {e}")
            raise
        except Exception as e:
            # logger.error(f"Неожиданная ошибка при сохранении калибровки: {e}")
            raise
    ...
    def _on_param_change(self, event=None):
        ...
        if self.mode == "edit_container" and self.editing_container_id:
            ...
            if container:
                ...
                if (abs(container.length - new_length) > 0.01 or
                        abs(container.width - new_width) > 0.01 or
                        abs(container.height - new_height) > 0.01):
    ...
    def _on_canvas_drag(self, event):
        ...
        if self.dragging_point_idx is not None:
            ...
            if 0 <= self.dragging_point_idx < len(self.drawing_points):
                old_x, old_y = self.drawing_points[self.dragging_point_idx]
                if abs(old_x - x) > 0.1 or abs(old_y - y) > 0.1:
    ...
    def _edit_camera_dialog(self):
        ...
        # Активность камеры
        active_var = tk.BooleanVar(value=camera.is_active)
        ttk.Checkbutton(main_frame, text="Камера активна", variable=active_var).grid(row=3, column=0, columnspan=2,
                                                                                     pady=5)
        ...
        def save():
            ...
            try:
                ...
                messagebox.showerror("Не найдено", str(e))
            except RepositoryError as e:
                messagebox.showerror("Ошибка БД", str(e))
            except Exception as e:
                messagebox.showerror("Ошибка", str(e))
    ...
    #                 'ground_points': container.ground_corners if container.ground_corners is not None else [],
    #                 'upper_points': container.upper_corners if container.upper_corners is not None else [],
    ...
    #             cid = int(cid_str)
    #
    #             ground_points = np.array(cont_data.get('ground_points', [])) if cont_data.get('ground_points') else None
    #             upper_points = np.array(cont_data.get('upper_points', [])) if cont_data.get('upper_points') else None
    #             image_points = np.array(cont_data['image_points']) if cont_data.get('image_points') else None
    #
